...
from model.cameras import getProjectionMatrix, getWorld2View2, focal2fov
...
class FaceDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        for dir in sub_dir:
            instance_dir = os.path.join(data_folder, subject_name, dir)
            assert os.path.exists(instance_dir), "Data directory {} is empty".format(instance_dir)

            flame_file = '{0}/{1}'.format(instance_dir, json_name)

            with open(flame_file, 'r') as f:
                flame_dict = json.load(f)
            intrinsics_params = flame_dict['intrinsics']
            intrinsic_matrix = np.zeros((3, 3))
            intrinsic_matrix[0, 0] = -intrinsics_params[0] * self.img_res[0]
            intrinsic_matrix[1, 1] = intrinsics_params[1] * self.img_res[1]
            intrinsic_matrix[2, 2] = 1
            intrinsic_matrix[0, 2] = intrinsics_params[2] * self.img_res[0]
            intrinsic_matrix[1, 2] = intrinsics_params[3] * self.img_res[1]
            self.intrinsic_matrix = intrinsic_matrix

            for frame in flame_dict['frames']:
        ...
        self.shape_params = torch.tensor(flame_dict['shape_params']).float().unsqueeze(0)
        focal_cxcy = flame_dict['intrinsics']
